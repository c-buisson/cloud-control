...
case "$1" in
  "kvm-control" )
    ...
    if [[ -z "$project" ]]; then
      echo -e "$txtbold Creating $1 project!$txtreset\n"
  ...
  "remove_chef_jobs" )
    ...
    if [[ $jobs == *"Docker"* ]]; then
      echo -e "\n$txtbold Removing unneeded Chef Docker jobs!$txtreset"
  ...
  "docker-control" )
    project=$(rd projects list |grep $1)
    if [[ -z "$project" ]]; then
      echo -e "$txtbold Creating $1 project and jobs!$txtreset"
